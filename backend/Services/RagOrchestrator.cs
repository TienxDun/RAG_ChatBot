...

        {
            ...
            var planningPrompt = $@"Bạn là chuyên gia phân tích yêu cầu và lập kế hoạch truy vấn SQL.
                ...
                2. Nếu câu hỏi liên quan đến database, hãy phân tích xem câu hỏi có bị mơ hồ, thiếu thông tin gom nhóm (GROUP BY) hoặc thống kê cụ thể hay không (ví dụ: 'top lỗi', 'sản lượng cao nhất'):
                   ...
                   - **CẤM TIỀN TỐ DATABASE:** Tuyệt đối KHÔNG thêm bất kỳ tiền tố tên cơ sở dữ liệu hoặc schema nào (như `DataBase.dbo.`, `Database.dbo.`, `XuongMay.dbo.`, `dbo.`) trước tên bảng. Chỉ viết tên bảng trực tiếp (ví dụ: `QTY_MAHANG_NGAYKIEM`). Việc sử dụng tiền tố `DataBase.dbo.` sẽ gây ra lỗi cú pháp nghiêm trọng do 'DataBase' là từ khóa hệ thống (Reserved Keyword) của SQL Server.
                   - **CÚ PHÁP GROUP BY BẮT BUỘC:** Nếu câu lệnh SQL `directSql` sử dụng mệnh đề `GROUP BY`, mọi cột xuất hiện trong danh sách `SELECT` mà không nằm trong các hàm gộp (như `SUM`, `COUNT`, `AVG`, `MAX`, `MIN`) BẮT BUỘC phải xuất hiện đầy đủ trong mệnh đề `GROUP BY`. TUYỆT ĐỐI KHÔNG để sót bất kỳ cột nào để tránh lỗi cú pháp 'is invalid in the select list because it is not contained in either an aggregate function or the GROUP BY clause' của SQL Server.

...
public class ExcelReportService
{
    ...
    public async Task<ExcelReportResult> ProcessExcelTemplateAsync(Stream stream, string fileName, string additionalQuery, string? collectionName, Func<RagStep, Task> onStep, Func<string, Task> onFinalChunk, CancellationToken ct)
    {
        try
        {
            ...
            // Tự động gán lại số thứ tự (STT) tăng dần từ 1 sau khi đã sắp xếp
            ...
            if (templateInfo.Type == TemplateType.Hierarchical)
            {
                _templateFiller.FillHierarchicalTemplate(
                    ...
                    templateMapping.ColumnFormats,
                    templateMapping.MetadataCellMappings);
            }
            else
            {
    ...
    private static void SortDataTableByDate(DataTable dt, List<FlattenedColumn> columns)
    {
        ...
        if (dateCol != null && dt.Columns.Contains(dateCol.UniqueKey))
        {
            try
            {
                ...
                // Reset lại Sort của DefaultView để tránh lỗi Index khi xóa cột tạm
